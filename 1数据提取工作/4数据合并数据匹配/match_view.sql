...
WITH pao2 AS (
 ...
 SELECT subject_id,hadm_id,icustay_id,charttime
  ...
  --以pao2为标准，匹配2小时以前的spo2数据，如果没有就为空，如果有多条，选择最近的一条，也就是选择lastRowSpO2=1的就OK了
    SELECT pao2.subject_id,pao2.hadm_id,pao2.icustay_id,pao2.charttime,pao2.pao2,spo2.spo2,spo2.charttime AS s_time,
      CASE WHEN pao2.charttime>spo2.charttime THEN pao2.charttime - spo2.charttime ELSE spo2.charttime - pao2.charttime END AS sc
    FROM pao2
      LEFT JOIN spo2
    ...
    SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowSpO2 FROM patvalue
),stg1 AS (
...
--,row_number() OVER (PARTITION BY pat.icustay_id,pat.charttime ORDER BY s2.charttime DESC ) AS lastRowHR
  ,CASE WHEN pat.charttime>s2.charttime THEN pat.charttime - s2.charttime ELSE s2.charttime - pat.charttime END AS sc
,s2.hr
FROM stg1 pat
  LEFT JOIN hr s2
...
WHERE pat.lastRowFiO2=1
   ...
   SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowHR FROM stg2
),stg3 AS (
...
WHERE pat.lastRowHR=1
    ...
    SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowTemp FROM stg3
),stg4 AS (
...
WHERE pat.lastRowTemp=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowNbps FROM stg4
),stg5 AS (
...
WHERE pat.lastRowNbps=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowNbpd FROM stg5
),stg6 AS (
...
WHERE pat.lastRowNbpd=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowNbpm FROM stg6
),stg7 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd,pat.nbpm
...
WHERE pat.lastRowNbpm=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowAbps FROM stg7
),stg8 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
,pat.nbpm,pat.abps
...
WHERE pat.lastRowAbps=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowAbpd FROM stg8
)
  ,stg9 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
...
WHERE pat.lastRowAbpd=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowAbpm FROM stg9
),stg10 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
...
WHERE pat.lastRowAbpm=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowRR FROM stg10
),stg11 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
...
WHERE pat.lastRowRR=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowTV FROM stg11
),stg12 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
...
WHERE pat.lastRowTV=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowPIP FROM stg12
),stg13 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
...
WHERE pat.lastRowPIP=1
   ...
   SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowPLAP FROM stg13
),stg14 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
...
WHERE pat.lastRowPLAP=1
  ...
  SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowMV FROM stg14
),stg15 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
...
WHERE pat.lastRowMV=1
   ...
   SELECT *,row_number() OVER ( PARTITION BY icustay_id, charttime ORDER BY sc ) AS lastRowMAP FROM stg15
),stg16 AS (
SELECT pat.subject_id,pat.hadm_id,pat.icustay_id,pat.charttime,pat.pao2,pat.spo2,pat.fio2,pat.hr,pat.temp,pat.nbps,pat.nbpd
